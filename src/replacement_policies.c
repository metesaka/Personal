                      ...
                      struct cache_system *cache_system, uint32_t set_idx, uint32_t tag)
{
    ...
    if (cl == NULL || cl->status == INVALID)
    {
        for (int i = 0; i < (cache_system->associativity * cache_system->num_sets)-1; i++) {
            replacement_policy->data[i] = replacement_policy->data[i+1];
        }
        replacement_policy->data[(cache_system->associativity * cache_system->num_sets) - 1] = tag;
    }
    else
    {
        for (int i = 0; i < (cache_system->associativity * cache_system->num_sets) - 1; i++) {
            if (replacement_policy->data[i] == tag) {
                for (int j = i; j < (cache_system->associativity * cache_system->num_sets) - 1; j++) {
                    replacement_policy->data[j] = replacement_policy->data[j + 1];
                }
                replacement_policy->data[(cache_system->associativity * cache_system->num_sets) - 1] = tag;
            }
...
struct replacement_policy *lru_replacement_policy_new(uint32_t sets, uint32_t associativity)
{
    ...
    // DONE TODO allocate any additional memory to store metadata here and assign to
    // lru_rp->data.
